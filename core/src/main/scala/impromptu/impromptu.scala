...
object Async {
  ...
  /** constructs a new asynchronous computation which should run after the specified prerequisites
    * 
    * @param deps     the prerequisite dependencies of the computation
...
/** an asynchronously computed value, much like a lazy [[Future]]
  * 
  * @param deps    the dependencies which must be computer prior to this [[Async]] value
...
class Async[+Return, Before] private (val deps: Seq[Async[_, _]],
  ...
  def apply()(implicit env: Async.Env[this.type]): Return =
    future.value.get.get.asInstanceOf[Return]

  /** the lazily-evaluated [[Future]] corresponding to this asynchronous value */
  lazy val future: Future[Return] =
    Future.sequence(deps.map(_.future)).map { _ => action(Async.AnyEnv) }
